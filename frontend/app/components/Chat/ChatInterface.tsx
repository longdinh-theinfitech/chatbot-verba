...
const ChatInterface: React.FC<ChatInterfaceProps> = ({
    ...
    const [allDocuments, setAllDocuments] = useState<DocumentPreview[] | null>([]);
    const [filterDocuments, setFilterDocuments] = useState<DocumentPreview[]>([]);
    ...
    const sendUserMessage = async () => {
        ...
        const context = filterDocuments.length > 0 ? filterDocuments : allDocuments;
        try {
    ...
    }, [currentDatacount, labels, filterLabels]);
    ...
    const fetchAllDocuments = async (_userInput?: string) => {
        try {
            ...
            addStatusMessage("Failed to fetch documents: " + error, "ERROR");
        }
    };
            ...
            <div className="bg-bg-alt-verba rounded-2xl flex flex-col h-[50vh] md:h-full w-full overflow-y-auto overflow-x-hidden relative">
                ...
                {selectedSetting == "Chat" && (
                    <div className="sticky flex flex-col gap-2 top-0 z-9 p-4 backdrop-blur-sm bg-opacity-30 bg-bg-alt-verba rounded-lg">
                        ...
                        <div className="flex gap-2">
                            <div className="dropdown dropdown-hover">
                                ...
                                <ul
                                    ...
                                    className="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52"
                                >
                                    {allDocuments && allDocuments.map((element, index) => (
                                        <li key={"Document" + index}>
                                            <a className="text-ellipsis"
                                                onClick={() => {
                                                    if (!filterDocuments.includes(element)) {
                                                        setFilterDocuments([...filterDocuments, element]);
                                                        setCurrentDatacount(filterDocuments.length + 1);
                                                    }
                                                    const dropdownElement =
                                                        document.activeElement as HTMLElement;
                                                    dropdownElement.blur();
                        ...
                        <div className="flex flex-wrap gap-2">
                            {filterDocuments && filterDocuments.map((document, index) => (
                                <VerbaButton
                ...
                <div className={`${selectedSetting === "Chat" ? "flex flex-col gap-3 p-4" : "hidden"}`}>
